...
public class AlgoEnumTotaleSumDistance extends AbstractAlgo {

    /**
     * La meilleure solution est déterminée en fonction de la somme des distances à la scène des rangées engagées.
     *
     * @param problem
     * @return
     */
    ...
    public Solution execute(AbstractProblem problem) {
        ...
        for (int i = 0; i < rowsRepartition.size(); i++) {
            ...
            // ou si pour un même nombre de de groupes sont placés mais que la distance totale à la scène est inférieure
            if (tmpSolution.getValue1().getTotalUnplacedGroups() < bestSolution.getValue1().getTotalUnplacedGroups()) {
                ...
                indexBestRowRepartition = i;
                    ...
                    && tmpSolution.getValue1().getSumDistance() < bestSolution.getValue1().getSumDistance()) {
                bestSolution = tmpSolution;
                indexBestRowRepartition = i;
            }
        }
            ...
            if (rowsCapacityLeft.get(i) >= reservations.get(indexReservation)) {
                ...
                if (currentSolution.getValue1().getTotalUnplacedGroups() < bestSolution.getValue1().getTotalUnplacedGroups()) {
                    bestSolution = currentSolution;
                        ...
                        && currentSolution.getValue1().getSumDistance() < bestSolution.getValue1().getSumDistance()) {
                    bestSolution = currentSolution;
                }
            }
        }
